...
class RNN(nn.Module):
    def __init__(self, num_features, num_classes, n_layers=1, hidden_dim=256, dropout=0.2):
        ...
        # x_example = torch.zeros(batch_size, input_size, hidden_dim).cuda()
        # factor = 2
        # self.emb = nn.Linear(input_size, hidden_dim)
        # self.lstm = xlstm('ms', x_example, factor=factor, depth=n_layers)
