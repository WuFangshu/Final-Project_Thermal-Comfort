...
class RNN(nn.Module):
    def __init__(self, num_features, num_classes, n_layers=1, hidden_dim=256, dropout=0.2):
        ...
        batch_size, seq_lenght, input_size = 4, 8, num_features
        x_example = torch.zeros(batch_size, input_size, hidden_dim).cuda() #xlstm
        factor = 2 #xlstm
        self.emb = nn.Linear(input_size, hidden_dim) #xlstm
        self.lstm = xlstm('ms', x_example, factor=factor, depth=n_layers) #xlstm
        self.dp_layer = nn.Dropout(dropout) #xlstm and lstm
        # self.lstm = nn.LSTM(num_features, hidden_dim, n_layers, batch_first=True) #lstm
        # if n_layers > 1:  #lstm
        #     self.lstm = self.lstm = nn.LSTM(num_features, hidden_dim, n_layers, batch_first=True, dropout=dropout)  #lstm
    ...
    def forward(self, x):
        #print(x.shape)
        # self.lstm.flatten_parameters() #use multi GPU capabilities #lstm
        # feat, (h_t, s_t) = self.lstm(x) #lstm
        hidden_x = self.emb(x) #xlstm
        feat = self.lstm(hidden_x) #xlstm
        x = feat[:, -1]
        if self.n_layers == 1:
            x = self.dp_layer(x)
        x = self.fc1(x)
