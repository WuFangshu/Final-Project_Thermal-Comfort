...
class TC_RNN_Module(pl.LightningModule):
    def __init__ (self, path, batch_size, learning_rate, worker, metrics, get_sequence_wise, sequence_size, cols, gpus, dropout, hidden, layers, preprocess, augmentation, skip, forecasting, scale, dataset, weight_decay=0.0, *args, **kwargs):
        super().__init__()
    ...
    def configure_optimizers(self):
        ...
        # Training parameters
        optimizer = torch.optim.Adam(train_param, lr=self.hparams.learning_rate, weight_decay=self.hparams.weight_decay)
        scheduler = {
    ...
    #         optimizer,
    #         lr_lambda=lambda step: decay ** step  # step 是 scheduler 内部计数
    #     )
    ...
    def training_step(self, batch, batch_idx):
        ...
        with open('logs/train.txt', 'a') as f:
